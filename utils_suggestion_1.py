...
def tournament_selection(pop, num_elites, num_parents, tm_size):
    ...
    for i in range(num_parents):
        best_indv = random.randrange(len(all))
        for j in range(tm_size):
            temp_indv = random.randrange(len(all))
            if all[temp_indv].fitness > all[best_indv].fitness:
                best_indv = temp_indv
        # store the best "num_parents" parents
        parents.append(all[best_indv])
        if all[best_indv] in others:
            others.remove(all[best_indv])
        else:
...
def crossover(pop, num_elites, num_parents, tm_size, source_img):
    ...
    """# combine the elites and children to create the next generation
    next_generation_indvs = elites + children
    next_gen_size = num_elites + num_parents
    # create the next population object
    next_generation = population(next_gen_size, gene_num, img_size, next_generation_indvs)"""
